...
public class AuthenticationFilter extends GenericFilterBean implements ApplicationContextAware {
  ...
  private Collection<com.zcxn.wp.cloud.auth.integration.IAuthenticator> authenticators;
  ...
  public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
    ...
    if(requestMatcher.matches(request)){
      //设置集成登录信息
      AuthenticationType integrationAuthentication = new AuthenticationType();
   ...
   */
  private void prepare(AuthenticationType integrationAuthentication) {

    //延迟加载认证器
    if(this.authenticators == null){
      synchronized (this){
        Map<String, com.zcxn.wp.cloud.auth.integration.IAuthenticator> integrationAuthenticatorMap = applicationContext.getBeansOfType(com.zcxn.wp.cloud.auth.integration.IAuthenticator.class);
    ...
    for (com.zcxn.wp.cloud.auth.integration.IAuthenticator authenticator: authenticators) {
   ...
   * @param integrationAuthentication
   */
  private void complete(AuthenticationType integrationAuthentication){
    for (com.zcxn.wp.cloud.auth.integration.IAuthenticator authenticator: authenticators) {
      if(authenticator.support(integrationAuthentication)){
